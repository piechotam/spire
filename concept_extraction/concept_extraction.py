...
def load_sae(checkpoint_path: str,
             device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu") -> torch.nn.Module:
    """
    ...
    """
    try:
        sae_state_dict = torch.load(checkpoint_path,
        ...
        sparse_autoencoder = SparseAutoencoder(n_input_features=_SAE_N_INPUT_FEATURES,
...
def prepare_image(image_path: str, preprocess: torchvision.transforms.transforms.Compose):
    """
...
def extract_concepts(n_concepts: int,
                     concept_names: tuple,
    ...
    indices_of_largest = np.argpartition(concept_activations_numpy, -n_concepts)[-n_concepts:]
    largest_indices_sorted = indices_of_largest[np.argsort(concept_activations_numpy[indices_of_largest])]
